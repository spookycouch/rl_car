...
def get_dynamic_bounds(step, max_steps=5_000_000, start=[0.3, 0.7], end=[0.1, 0.9]):
    ratio = min(step / max_steps, 1.0)
    value = [start[i] + ratio * (end[i] - start[i]) for i in range(len(start))]
    return value
...
class CarEnv(gym.Env):
    ...
    def __init__(
        ...
        self._object = create_box([0.035,0.035,0.035], [0.5,0.5,0.05], [1,0,0,1], mass=0.1)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.__num_steps = 0
        self.__episode_reward = 0.0
        min_bound, max_bound = get_dynamic_bounds(self.__total_steps)
    ...
    def step(self, action):        
        ...
        for _ in range(24):
            p.setJointMotorControl2(
                ...
                0,
                p.VELOCITY_CONTROL,
                targetVelocity=target_velocities[0] + target_velocities[1]
            )
            p.setJointMotorControl2(
                self._car,
                1,
        ...
        # reward = -(dist_object * 0.02 + dist_goal * 0.01)
        # done = dist_goal < 0.05
        reward = -(dist_object * 0.01)
        done = False
        if done:
            reward += 1.0

        truncated = False
        if self.__num_steps > 150: # 15s
            min_bound, max_bound = get_dynamic_bounds(self.__total_steps)
            p.resetBasePositionAndOrientation(
                self._car,
                list(np.random.uniform(min_bound, max_bound, (2))) + [0.05],
                Rotation.from_euler('z', np.random.uniform(0, 2*np.pi)).as_quat()
            )
            truncated = True
        info = {}
